...
class HangmanGame(object):                      # Single responsibility. The class should only implement game's logic.
    """
    ...
    def choose_word(self) -> str:
        ...
        :return: str
        """
        timeout = 10                            # Open-closed. If we wanted to use another method of getting
        try:                                    # random word (i.e. from database), we would need to modify the code.
            response = requests.get('https://random-word-api.herokuapp.com/word', timeout=timeout)
        except requests.exceptions.Timeout:
            sys.stdout.write('Server did not respond in {timeout} seconds. We are sorry :(\n'.format(timeout=timeout))
            sys.exit(1)
        except requests.exceptions.RequestException as error:
            sys.stdout.write('An error occurred: {error}. We are sorry :(\n'.format(error=error))
            sys.exit(1)
    ...
    def play(self) -> None:
        ...
        while not self.game_end():
            sys.stdout.write('{word} guess a character: '.format(word=self.make_hidden_word()))  # Single responsibility
            sys.stdout.flush()                                                                   # i/o logic should be
            char: str = sys.stdin.read(2).rstrip('\n').lower()                                   # in separate class.
